...
public class purchaseSummaryAdapter extends BaseAdapter {
    Context context;
    PurchaseMaster[] purchaseMasters =null;

    public purchaseSummaryAdapter(Context context, PurchaseMaster[] purchaseMasters){
   ...
   // int pos=0;
    ...
    public Object getItem(int i) {
        return purchaseMasters[i];
    }

    @Override
    public int getCount() {
        return purchaseMasters.length;
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if(view==null){
            view= LayoutInflater.from(context).inflate(R.layout.purchasesummary_single,viewGroup,false);
        }
        if(i%2==0){
    ...
    public String sumofNet(){
        double sum=0;
        for(PurchaseMaster s: purchaseMasters ){
            sum+=s.getNetAmount();
        }
